...
- [ ] **T-144 [P1] Move attach/scan/button off the GUI render thread** (`crate: trainlab-gui`)
  - **DEFERRED:** `find_inject_connect` (~4.5 s poll), value scans, and
    `run_cheat_commands` (with `Wait`) still run inside `update()`, freezing the UI.
  - Fix: run on a worker thread and report progress via the event bus.
