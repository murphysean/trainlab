...
- **Hooking is cleaner at the runtime level** — patch in-memory IL, force a
  re-JIT, or hook Mono's method dispatch. Distinct from native x86 caves.
- Debug with `dnSpy`/`ilspy` at the IL/source level.

This is why a Mono game like Urbek is a great first target: you can *read* the
logic directly instead of reversing assembly.
