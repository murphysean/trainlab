...
| Tool | What it does |
|------|--------------|
| `write` | Write bytes (with undo snapshot, validated to writable regions) |
| `install_cave` | Emit shellcode, place a cave, write the jmp, save original bytes |
| `restore` / `undo` | Revert to saved originals |
...
(`Assembly-CSharp.dll`) JIT-compiled at runtime. Two implications:

1. **Decompilation is lossless-ish.** A .NET decompiler (`ilspycmd`) reconstructs
   near-original C# with real method/field/class names. Far better than C/C++.
2. **Hooking is cleaner at the runtime level.** Instead of patching raw x86
   bytes, hook Mono's method dispatch or patch the in-memory IL and force a
   re-JIT. This is a distinct, elegant technique from native caves.

For a Mono game, `dnSpy`/`ilspy` can attach and debug at the IL/source level,
which beats both GDB and x64dbg for understanding game logic.
...
| `trainlab-inject` | TCP server, in-process mem | Windows allocate/free; injection entrypoints; Mono hooks |
...
| *(new)* `trainlab-mono` | — | Mono runtime walking, IL hooking |
